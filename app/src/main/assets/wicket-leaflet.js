...
Wkt.Wkt.prototype.deconstruct = function (obj) {
    ...
    // L.Marker ////////////////////////////////////////////////////////////////
    if (obj.constructor === L.Marker || obj.constructor === L.marker || obj.constructor === L.Marker.Touch) {
        return {
            type: 'point',
            components: [{
                x: obj.getLatLng().lng,
    ...
    if (obj.constructor === L.Polygon || obj.constructor === L.polygon) {
        ...
        if (tmp.constructor === Array) {
            for (ring = 0; ring < tmp.length; ring += 1) {
                verts = [];
                // First, we deal with the boundary points
                for (i = 0; i < tmp[ring].length; i += 1) {
                    verts.push({ // Add the coordinate
                        x: tmp[ring][i].lng,
                        y: tmp[ring][i].lat
                    });
                }
